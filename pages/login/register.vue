	...
	export default {
		...
		methods: {
			...
			getVerCode() {
				...
				console.log("获取验证码")
				uni.request({
					url: App.requestIp + 'user/getVerificationCode/' + this.phoneData,
					method: 'GET',
					success: (res) => {
						console.log(res);
						uni.setStorage({
			...
			startReg() {
				...
				uni.request({
					url: App.requestIp + "/user/validateCode/" + this.phoneData + '/' + this.verCode,
					method: 'GET',
					header: {
					...
					success: (res) => {
						...
						console.log("笔友" + this.guid())
						if (res.data.msg === 'ok') {
							uni.request({
								url: App.requestIp + "/user/register",
								method: 'POST',
								header: {
									token: uni.getStorageSync('registerToken')
								},
